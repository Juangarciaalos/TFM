...
def generate_individual_plots(args):
    ...
    def save_plot(dataframe, suffix, is_zoom=False):
        ...
        plt.plot(dataframe['global_step'], dataframe['reserved_mb'], 
                 ...
                 marker='.' if is_zoom else None) # Marcador solo en zoom
        
        plt.plot(dataframe['global_step'], dataframe['allocated_mb'], 
                 label='Allocated (Uso Real)', color='#1f77b4', 
                 linewidth=2, marker='o' if is_zoom else None) # Marcador solo en zoom
    ...
    print(f"Gráficas individuales (Full y Zoom) guardadas en salidas/graficas/")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generador de gráficas dobles para BERT (TFM)")
